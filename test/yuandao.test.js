...
describe("YuanDao", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Should set the correct roles", async function () {
      expect(
        await yuanDao.hasRole(
          await yuanDao.DEFAULT_ADMIN_ROLE(),
          deployer.address
        )
      ).to.be.true;
      expect(
        await yuanDao.hasRole(await yuanDao.PROPOSER_ROLE(), deployer.address)
      ).to.be.true;
      expect(
        await yuanDao.hasRole(await yuanDao.EXECUTOR_ROLE(), deployer.address)
      ).to.be.true;
    });
  });
  ...
  describe("Voting", function () {
    ...
    it("Should cast a vote", async function () {
      ...
      const { _optionAVotes, _optionBVotes, _totalVotes } =
        await yuanDao.getProposalVotes(proposalId);
    ...
    it("Should revert if voting on a non-active proposal", async function () {
      ...
      await expect(
        yuanDao.castVote(proposalId, support, weight)
  ...
  describe("Execute", function () {
    ...
    it("should execute a proposal", async function () {
      ...
      const { proposer, voteStart, voteDuration, executed, canceled } =
        await yuanDao.getProposalDetails(proposalId);
      expect(executed).to.be.true;
    });

    it("should revert if called by non-user1 and non-admin", async function () {
      await expect(
        yuanDao.connect(user2).execute(proposalId)
      ).to.be.revertedWithCustomError(yuanDao, "GovernorUnauthorizedProposer");
    });

    it("should revert if proposal is already executed", async function () {
      await yuanDao.execute(proposalId);
      await expect(yuanDao.execute(proposalId)).to.be.revertedWithCustomError(
        yuanDao,
        "ProposalAlreadyExecutedOrCancelled"
      );
    });
